 ...
 * He also has an excellent explanation of the algorithm.
 *
 *
 * The image is first translated into a java BitSet, and then the algorithm
 * is repeatedly run until no more deletions are possible.
 *
 ...
 *      Find m1, m2, A and B:
 *          m1:
 *              if iter == false -> m1 = p1 && p3 && p5
 *              if iter == true  -> m1 = p1 && p3 && p7
 *          m2:
 *              if iter == false -> m2 = p3 && p5 && p7
 *              if iter == true  -> m2 = p1 && p5 && p7
 *          A:
 *              A = the number of 'false' => 'true' transitions, considering p1 -> p2, p2 -> p3, p3 -> p4, ... , p8 -> p1
 *          B:
 *              B = is the number of p's that are true in p1, p2, ... , p8
 *
 *          Evaluate the Result:
 *              Result = (A == 1 && (B >= 2 && B <= 6) && m1 == false && m2 == false)
 *
 *      If the Result is true we clear p0 (set it to false, or 0) and slide the matrix to the next index.
 *      If the Result is false we leave p0 alone and slide the matrix to the next index.
 *
...
public class ZhangSuenThinning {
    ...
    /**
     * Method for thinning the given binary image.
    ...
    public static Mat thinning(final Mat binaryInputImage) {
        ...
        binaryInputImage.get(0,0, bytes);
    ...
    private static boolean thinningIteration(BitSet imageBits, boolean iter, int rows, int cols) {
        ...
        for (int row = 1; row < rows - 1; row++) { // rows == height == y
            for (int col = 1; col < cols - 1; col++) { // cols == width == x
